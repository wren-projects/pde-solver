...
class TTD[DType: np.floating](NDArrayOperatorsMixin):
    ...
    def round(self, epsilon: DType | float = DEFAULT_EPSILON) -> None:
        ...
        for k in range(d, 1, -1):  # for k = d to 2 step -1
            ...
            q, r = map(
                np.transpose, np.linalg.qr(core.reshape((alpha_k1, i_k * beta_k)).T)
            )
            # 𝐆ₖ(βₖ₋₁; iₖβₖ) = 𝐐
    ...
    @override
    def __add__(self, other: TTD[DType] | NDArray[DType]) -> TTD[DType]:
        """Add two TTD objects."""
        return self.add(self, other)

    @override
    def __iadd__(self, other: TTD[DType] | NDArray[DType]) -> TTD[DType]:
