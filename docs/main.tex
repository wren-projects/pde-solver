...
\begin{verbatim}
    def from_ndarray[DT: np.floating](
        ...
        delta = _truncation_parameter(array, epsilon)

        shape: tuple[int, ...] = array.shape

        remaining_tensor = array.reshape((shape[0], -1))

        r = 1

        cores: TTD.DataType[DT] = []

        for n in shape[:-1]:
            remaining_tensor = remaining_tensor.reshape((r * n, -1))

            u, s, v_t = _delta_truncated_svd(remaining_tensor, delta)
            ...
            remaining_tensor = cast(MatrixType[DT], s[:, None] * v_t)
...
\paragraph{Code slot (NumPy).}
\begin{verbatim}
    def _round(self, _epsilon: DType | float = DEFAULT_EPSILON) -> None:
        """
        Round the TTD object by decreasing ranks.

        Uses SVD for compression. Ensures that the ranks of ��̃ are maximally
        reduces, while ensuring ||A - ̂��̃||ᶠ ≤ ε ⋅ ||A||ᶠ.

        Operates on the TTD object in-place. See also :func:`TTD.rounded`.
        ...
        for k in range(d, 1, -1):  # for k = d to 2 step -1
            ...
            cores[k - 2] = _mode_3_contraction(cores[k - 2], r)

        u: MatrixType[DType]
        s: VectorType[DType]
        v_t: MatrixType[DType]

        for k in range(1, d):  # for k = 1 to d-1
            core = cores[k - 1]
            beta_k1, i_k, beta_k = core.shape
            u, s, v_t = _delta_truncated_svd(core.reshape(beta_k1 * i_k, beta_k))
            cores[k - 1] = u.reshape((beta_k1, i_k, -1))
            cores[k] = _mode_1_contraction(cores[k], np.multiply(v_t, s))
\end{verbatim}
...
\paragraph{Code slot.}
\begin{verbatim}
def _frobenius_norm[DT: np.floating](tensor: NDArray[DT]) -> DT:
