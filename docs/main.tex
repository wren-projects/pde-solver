...
multiplication.
\cite{Oseledets2009TreeTucker,Oseledets2011TT}
...
The explicit finite-difference method approximates the time derivative
...
The implemented numerical methods are verified using analytical
solutions of the governing partial differential equations and
convergence studies based on step-size halving.

Analytical solutions provide an exact reference against which the
numerical approximation can be compared, while step-size halving
verifies that the implemented method exhibits the expected order of
convergence. Together, these tests provide evidence for both the
correctness of the implementation and the consistency of the numerical
discretization \cite{LeVeque2007}.

%==================================================
\subsection{Validation using analytical solutions}
%==================================================

The primary verification procedure compares the numerical solution
against an analytical solution of the three-dimensional heat equation.
The analytical solution used in the test cases is taken from Herman
\cite{HermanPDE} and corresponds to the homogeneous heat equation
...
posed on the unit cube with homogeneous Dirichlet boundary conditions.
...
where $l$, $m$ and $n$ denote the spatial modes.

For each test case, the numerical solution is evolved to a prescribed
time and compared with the corresponding analytical solution. The
difference between the two solutions provides a direct measure of the
accuracy of the implemented numerical method.

%==================================================
\subsection{Convergence study using step-size halving}
%==================================================

In addition to comparison with analytical solutions, the implemented
methods are verified using a convergence study based on step-size
halving.

The same problem is solved repeatedly while reducing the spatial and/or
temporal discretization by a factor of two. For a numerical method of
order $p$, the discretization error satisfies
...
where $h$ denotes the discretization size and $C$ is independent of
$h$ \cite{LeVeque2007}.
...
which allows the observed convergence order to be estimated as
...
Agreement between the observed and theoretical convergence orders
provides additional verification that the numerical implementation is
consistent with the underlying finite-difference discretization.
