...
Each core of the operator interacts only with the corresponding core of the
vector at the same dimension.
...
the TT cores must first be permuted (see mode permutation section)
...
%==================================================
\section{TT-Matrix Format}
%==================================================

\subsection{TT-Matrix representation}

\begin{definition}[TT-matrix]

A linear operator
\[
A \in
\mathbb{R}^{(n_1 \cdots n_d) \times (m_1 \cdots m_d)}
\]
is said to be in TT-matrix format if it is represented by
matrix-valued functions
...
A TT-matrix represents a high-dimensional linear operator
as a sequence of small core tensors.

Each core connects input indices $j_k$ and output indices $i_k$
at dimension $k$, while also propagating low-rank coupling
information via auxiliary indices.

This representation reduces storage from exponential
$\mathcal{O}(n^d m^d)$ to polynomial $\mathcal{O}(d n m R^2)$.

%==================================================

\subsection{Differential operators in TT format}
%==================================================

\subsubsection{Gradient operator}

\begin{definition}[Gradient in TT format]

Let $A \in \mathbb{R}^{n_1 \times \cdots \times n_d}$ be a TT tensor.

For each $k = 1,\dots,d$, define a TT-matrix operator
\[
D^{(k)} =
I \otimes \cdots \otimes D_k \otimes \cdots \otimes I,
\]
where $D_k \in \mathbb{R}^{n_k \times n_k}$ is a 1D differentiation matrix.

The partial derivative is defined by
\[
\frac{\partial A}{\partial x_k}
=
D^{(k)} A,
\]
where the product is computed using TT matrix-vector multiplication.
...
Each derivative acts only along one dimension.

The TT-matrix $D^{(k)}$ inserts the differentiation matrix $D_k$
into the $k$-th position of a Kronecker product.

Applying the operator corresponds to TT matrix-vector multiplication,
which preserves the tensor structure.

\paragraph{Important note.}

Although the operator $D^{(k)}$ has low TT-rank,
the resulting tensor $D^{(k)}A$ typically has increased TT-ranks.

Therefore, rounding (\cref{alg:rounding}) is required.

\subsubsection{Laplace operator}

\begin{definition}[Laplace operator in TT format]
...
Each term is represented as a TT-matrix
\[
L^{(k)} =
I \otimes \cdots \otimes D_k^{(2)} \otimes \cdots \otimes I,
\]
where $D_k^{(2)}$ is a second-order differentiation matrix.

Thus,
\[
\Delta = \sum_{k=1}^d L^{(k)}.
\]
...
The Laplacian is a sum of independent second derivatives.

Each term has a simple tensor-product structure,
which admits a low-rank TT representation.

The sum increases TT-ranks linearly in $d$,
and rounding is applied after operator evaluation.

%==================================================
\subsection{TT-matrix applied to TT tensor}
\label{sec:tt_matvec}
%==================================================

\begin{definition}[TT-matrix times TT tensor]

Let a linear operator $A$ be given in TT-matrix format with cores
\[
A_k :
\{1,\dots,n_k\}\times\{1,\dots,m_k\}
\to
...
and let a tensor $X$ be given in TT format with cores
\[
G_k :
\{1,\dots,m_k\}
\to
...
The product $Y = AX$ is defined componentwise by
\[
Y(i_1,\dots,i_d)
=
\sum_{j_1,\dots,j_d}
...
The resulting tensor $Y$ admits a TT representation with cores
\[
\widetilde{G}_k(i_k)
=
\sum_{j_k=1}^{m_k}
...
\begin{algorithm}[ht]
  \caption{TT-matrix applied to TT tensor}
  \label{alg:tt_matvec}
  \begin{algorithmic}[1]
    \Require TT-matrix cores $A_1,\dots,A_d$,
    TT cores $G_1,\dots,G_d$
    \Ensure TT cores $\widetilde{G}_1,\dots,\widetilde{G}_d$
    ...
    \State Apply TT rounding (\cref{alg:rounding}) to $\widetilde{G}_1,\dots,\widetilde{G}_d$
...
The operator acts independently on each dimension.

At dimension $k$, we combine:

\begin{itemize}
  \item the operator slice $A_k(i_k,j_k)$,
  \item the tensor slice $G_k(j_k)$,
\end{itemize}

and sum over the index $j_k$.

The Kronecker product couples the operator rank $R_k$
with the tensor rank $r_k$, producing new ranks
\[
\widetilde{r}_k \approx R_k r_k.
\]

\paragraph{Important note.}

The TT-ranks grow multiplicatively during this operation.

Therefore, rounding (\cref{alg:rounding}) is required
to obtain a compact representation and control the error.

\paragraph{Complexity.}

If $n_k \le n$, $m_k \le n$, and all ranks are bounded by $r$ and $R$,
then the computational cost is
\[
\mathcal{O}(d n R^2 r^2).
\]

\paragraph{Code slot.}
\begin{minted}{python}
# TODO: TT-matrix x TT-tensor
# - loop over cores
# - contract over j_k
# - use kron(A_k slice, G_k slice)
# - round result
\end{minted}
