...
\paragraph{Example (inner product complexity).}
Let $A, B \in \mathbb{R}^{n \times n \times n}$.
...
In TT format, compute recursively:
\[
M_k = \sum_{i_k} G_k(i_k)^\top M_{k-1} H_k(i_k).
\]

\paragraph{Explanation (informal).}
The computation proceeds dimension by dimension. At each step, only small
matrices are manipulated.

\paragraph{Explanation (informal).}
If TT-ranks are bounded by $r$, the complexity becomes
$\mathcal{O}(d n r^3)$ instead of $\mathcal{O}(n^d)$.
