...
\hypersetup{
  ...
  pdftitle={Tensor Train (TT) Decomposition and PDE Solvers},
  pdfauthor={Pavel Altmann, Jakub Kopčil, Eva Trejbalová (WREN)}
}
\urlstyle{same}
...
At step $k$ we view the tensor as a matrix that separates the first $k$
variables from the remaining ones. The SVD extracts the minimal interaction
dimension $r_k$. Each singular vector block becomes the matrix $G_k(i_k)$. Thus,
TT-SVD sequentially extracts the ``interaction ranks'' between variable groups.
...
reduces to an inner product computation (see below). In practice,
orthogonalization often improves stability.
...
interaction
between the first $k$ dimensions. Since the boundary ranks satisfy $r_0=s_0=1$,
the final matrix $M_d$ is a scalar. This reduces exponential complexity to
...
\begin{definition}[Kronecker sum representation]
  For the Poisson equation with constant coefficients, the discrete operator has
  the form
  ...
  where $L \in \mathbb{R}^{n \times n}$ is the 1D discrete Laplacian, end $I$ is
  the identity matrix.
...
The $d$-dimensional Laplacian is simply the sum of independent 1D Laplacians.
Each term acts only on one variable, while leaving others unchanged. This
separability means the operator has a low-rank tensor structure. Instead of
storing a matrix of size $n^d \times n^d$, we store $d$ small matrices of size
$n \times n$. This is precisely the type of structure that the TT
format can exploit.
