...
This modularity improves testability and allows us to implement other versions of tensor compressions and have them work "out of the box."

While Solver, in theory, works on any NDArray implementation (which is selected by the user via dependency injection), in practice, only implementations that alter the usage of some specific methods are useful. The NDArray is interacted with only in the solvers themselves (where usually addition/subtraction/multiplication is all that is used), in BoundaryConditions (where setting slices is used), and in Operators (where a few rather specific methods are called).
